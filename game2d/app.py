...
class GameApp(kivy.app.App):
    ...
    def __init__(self, **keywords):
        ...
        **You will never call the constructor or run yourself**.  That is handled for
        ...
        assert x is None or type(x) in [
            int,
            float,
        ], "left edge %s is not a number" % repr(x)
        assert y is None or type(y) in [
            int,
            float,
        ], "top edge %s is not a number" % repr(y)
        ...
        assert x is None or type(x) in [
            int,
            float,
        ], "right edge %s is not a number" % repr(x)
        assert y is None or type(y) in [
            int,
            float,
        ], "bottom edge %s is not a number" % repr(y)
