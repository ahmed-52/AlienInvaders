...
class GSprite(GRectangle):
    ...
    def source(self, value):
        assert value is None or GameApp.is_image(
            value
        ), "%s is not an image file" % repr(value)
        self._source = value
        if self._defined:
            self._reset()
    ...
    def format(self, value):
        assert (
            type(value) in [tuple, list] and len(value) == 2
        ), "%s does is not a tuple pair" % repr(value)
        assert (
            type(value[0]) == int and type(value[1]) == int
        ), "%s does not have int values" % repr(value)
        assert value[0] > 0 and value[1] > 0, "%s does not have valid values" % repr(
            value
        )
    ...
    def _setFormat(self, value):
        ...
        """
        assert (
            type(value) == tuple and len(value) == 2
        ), "%s does is not a tuple pair" % repr(value)
        assert (
            type(value[0]) == int and type(value[1]) == int
        ), "%s does not have int values" % repr(value)
    ...
    def _reset(self):
        ...
        if texture:
            ...
            for row in range(self._format[0]):
                ...
                for col in range(self._format[1]):
                    self._images[row * self._format[1] + col] = texture.get_region(
                        int(tx),
                        texture.height - int(ty) - int(height),
                        int(width),
                        int(height),
                    )
                    tx += width
        ...
        self._bounds = Rectangle(
            pos=(x, y), size=(self.width, self.height), texture=self._texture
        )
