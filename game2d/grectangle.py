...
class GLabel(GRectangle):
    ...
    def halign(self, value):
        assert value in (
            ...
            "right",
            "center",
        ), "value %s is not a valid horizontal alignment" % repr(value)
        self._halign = value
    ...
    def valign(self, value):
        assert value in (
            ...
            "middle",
            "bottom",
        ), "value %s is not a valid vertical alignment" % repr(value)
        self._valign = value
    ...
    def left(self):
        ...
        **Invariant**: Must be an int or float.
        ...
        p0 = self.matrix._transform(
            self.x - self.width / 2.0, self.y - self.height / 2.0
        )[0]
        p1 = self.matrix._transform(
            self.x + self.width / 2.0, self.y - self.height / 2.0
        )[0]
        p2 = self.matrix._transform(
            self.x + self.width / 2.0, self.y + self.height / 2.0
        )[0]
        p3 = self.matrix._transform(
            self.x - self.width / 2.0, self.y + self.height / 2.0
        )[0]
