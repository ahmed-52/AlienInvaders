...
def in_triangle(p, t):
    ...
    """
    return (
        same_side(p, t[0:2], t[2:4], t[4:6])
        and same_side(p, t[2:4], t[0:2], t[4:6])
        and same_side(p, t[4:6], t[0:2], t[2:4])
    )
...
def is_point_tuple(t, minsize):
    ...
    try:
        ...
        return (
            len(t) % 2 == 0
            and len(t) >= 2 * minsize
            and reduce(lambda x, y: x and y, map(lambda z: type(z) in [int, float], t))
        )
    except:
        return False
...
class GPath(GObject):
    ...
    def points(self, value):
        assert is_point_tuple(
            value, 2
        ), "value %s is not a valid list of points" % repr(value)
    ...
    def near(self, point):
        ...
        for ii in range(size - 1):
            ...
            if p == q:
                test = (
                    np.sqrt((q[0] - x) * (q[0] - x) + (q[1] - y) * (q[1] - y)) < epsilon
                )
            else:
                num = abs(
                    (q[0] - p[0]) * x - (q[1] - p[1]) * y + q[0] * p[1] - p[0] * q[1]
                )
                den = np.sqrt(
    ...
    def _reset(self):
        ...
        if not self._linecolor is None:
            ...
            line = Line(
                points=self.points, cap="round", joint="round", width=self.linewidth
            )
...
class GTriangle(GPath):
    ...
    def _reset(self):
        ...
        if self.linewidth > 0:
            line = Line(
                points=self.points, joint="miter", close=True, width=self.linewidth
            )
            if not self._linecolor is None:
                self._cache.add(self._linecolor)
            self._cache.add(line)
...
class GPolygon(GPath):
    ...
    def source(self, value):
        ...
        assert value is None or GameApp.is_image(
            value
        ), "value %s is not an image file" % repr(value)
        self._source = value
        if self._defined:
            self._reset()
    ...
    def source_width(self, value):
        assert value is None or type(value) in [
            int,
            float,
        ], "value %s is not a valid width" % repr(value)
    ...
    def source_height(self, value):
        assert value is None or _is_num(value), "value %s is not a valid width" % repr(
            value
        )
    ...
    def __init__(self, **keywords):
        ...
        self.source_width = (
            keywords["source_width"] if "source_width" in keywords else None
        )
        self.source_height = (
            keywords["source_height"] if "source_height" in keywords else None
        )
        GObject.__init__(self, **keywords)
        self._reset()
        self._defined = True
    ...
    def _make_mesh(self):
        ...
        try:
            ...
            texture.wrap = "repeat"
            tw = (
                float(texture.width) if self.source_width is None else self.source_width
            )
            th = (
                float(texture.height)
                if self.source_height is None
                else self.source_height
            )
            ...
            self._mesh = Mesh(
                vertices=verts, indices=range(size + 2), mode="triangle_fan"
            )
